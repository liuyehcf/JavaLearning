package org.liuyehcf.compile.grammar.regex.symbol;
...
public class EscapedUtil {
    ...
    private static void initializeEscaped_any() {
        escaped_any = Collections.unmodifiableList(
                Arrays.asList(
                        Symbol.getAlphabetSymbolWithChar('.')));
    }
    ...
    private static void initializeEscaped_d() {
        ...
        for (char c = '0'; c <= '9'; c++) {
            symbols.add(Symbol.getAlphabetSymbolWithChar(c));
        }
        escaped_d = Collections.unmodifiableList(symbols);
    }

    private static void initializeEscaped_D() {
        Set<Symbol> symbols = new HashSet<>(Symbol.getAlphabetSymbols());
    ...
    private static void initializeEscaped_w() {
        ...
        symbols.add(Symbol.getAlphabetSymbolWithChar('_'));
