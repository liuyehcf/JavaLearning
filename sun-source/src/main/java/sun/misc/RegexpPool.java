...
/**
 * A class to represent a pool of regex expressions.  A string
 * can be matched against the whole pool all at once.  It is much
 * faster than doing individual regex expression matches one-by-one.
 *
...
public class RegexpPool {
    ...
    /**
     * Delete the regex expression and its target.
     *
     * @param re The regex expression to be deleted from the pool.
     ...
     * regex expressions that were added to the pool <i>after</i>
     * the last regex expression that matched in the last call
     ...
     * at all regex expressions in the pool.  match(s); is equivalent
     * to reset(); matchNext(s);
     * <p><b>Multithreading note:</b> reset/nextMatch leave state in the
     * regex expression pool.  If multiple threads could be using this
...
/* A node in a regex expression finite state machine. */
class RegexpNode {
    char c;
